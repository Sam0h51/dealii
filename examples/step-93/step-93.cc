...
template <>
double
Region_Indicator<2>::value(const Point<2>                     &p,
                           [[maybe_unused]] const unsigned int component) const
{
...
void Step3::setup_system()
{
  ...
  // Here, I loop over the cells looking for ones which are not on the boundary.
  // I do this because I thought that boundary cells would be set to 0 with
  // Dirichlet boundary conditions. Since I am using FE_DGQ elements, I don't
  // actually need to worry about this, so this could just be a loop over the
  // first n active cells.
  //
  // Once a cell is found, I set the FE_System index to 1, which corresponds to
  // the system with 2 FE_Q elements and one FE_DGQ element. Then, I call
  // distribute_dofs.
  ...
  // component. This will extract only the dofs from the third component of the
  // FE system.
  ...
  // my non-local dof. Then, I check if the l component of my fe system is
  // non-zero at this quadrature point. If both of these are true, then I add an
  // entry to the sparse matrix at the (nonlocal dof index, l dof index) entry
  // and the (l dof index, nonlocal dof index) entry (because the block system I
...
// index 1. It then loops over all dof indices and then all quadrature points of
// that dof. The goal is to determine the global index of the n dofs that we
      ...
      if (cell->active_fe_index() == 1)
        {
          ...
          // I use this value to add up all values at each quadrature point. If
          // this is ever non-zero, then I have found an index where the DGQ
          // element is set to 1.
          double total_value;
          ...
          // For this program, I only need 4 non-local dofs, so we stop the loop
          // once the indices for these have been found.
